...
**Verification note:** verified against a real backend (native Postgres, `bun run dev`) via a full browser walkthrough — login, quick-add tap and long-press amount edit, full add/edit form with numeric-keypad amount input, inline recategorise, swipe-to-delete with undo, search and category filters, and a 600-row seeded local dataset scrolled smoothly via the infinite-scroll pagination. Three real bugs were found and fixed in this pass: the login route leaked a raw plain-text error instead of JSON when `AUTH_PASSWORD_HASH` failed to parse; swipe-to-delete and tap-to-edit shared a pointer handler in a way that made a delete-swipe also reopen the editor for the deleted row; and the quick-add settings editor overflowed its modal, hiding the account field behind a horizontal scrollbar. Not covered: an actual offline/airplane-mode device test and a real Android build (Capacitor) — those still need a physical device or emulator pass.
